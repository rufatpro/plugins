...
IDE plugins and extensions. Platforms live in subdirectories:
...
|--------|----------|-----|--------|---------|------|
| [AI Chat File Links](jetbrains/ai-chat-file-links/) | JetBrains (PyCharm, IDEA, …) | `com.aichat.filelinks` | 0.2.0 | Opens files from **AI Chat** when URL / “file does not exist” errors occur; **Dismiss error notification when the file opens successfully** (Settings → Tools → AI Chat File Links) closes the error balloon after open | [readme.md](jetbrains/ai-chat-file-links/readme.md) · [readme_ru.md](jetbrains/ai-chat-file-links/readme_ru.md) |
...
**Purpose:** workaround for broken file links in **JetBrains AI Chat** with the **Cursor** agent (and others via [ACP](https://agentclientprotocol.com/)) — mainly on **Windows**. Developed with **AI assistance** (Cursor / LLM).

**Problem:** the agent response contains a relative path (e.g. `src/app/settings.py` or `docs/readme.md`), the file exists under the project root, but the IDE does not open it on click (URL error or “file does not exist”).
...
The “broken agent chat file links” scenario for VS Code/Cursor is **not implemented** in this repo yet; when an extension is added, it will appear in the catalog table and get a build section here.
